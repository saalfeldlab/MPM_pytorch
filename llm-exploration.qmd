...
::: {.callout-warning}
## Currently Exploring
**Best so far: R² = 0.574** with `1280×3 @ omega=35, lr=2E-5, 200k steps` (60.1 min)

Key findings (iterations 37-39):

- Prior optimal config (1280×4, omega=50) **FAILED** on this dataset (R²=0.175)
- C-optimal config (640×3, omega=25) performed WORSE (R²=0.130)
- **Depth=3 + high capacity critical**: 1280×3 @ omega=35 achieved R²=0.574
:::

| Finding | Value |
|---------|-------|
| Depth ceiling | 3 layers (not 4 like prior) |
| Capacity requirement | 1280 (confirmed, 640 fails) |
| Optimal omega_f | ~35 (lower than prior 50) |

::: {.panel-tabset}

#### Video
{{< video log/Claude_exploration/instruction_multimaterial_1_discs_3types/videos/iter_039_multimaterial_1_discs_3types_Claude.mp4 >}}

#### Final Field
![](log/Claude_exploration/instruction_multimaterial_1_discs_3types/final_field/final_field_S_20260128_132852.png){.lightbox}
...
2. **omega_f scales with field complexity**: Jp(5-10) < F(12) < C(25) < S(35)
3. **Capacity requirements vary**: F(256) < Jp(512) < C(640) < S(1280)
4. **Learning rate sensitivity differs**: F has wide tolerance, C/S need slower learning
5. **Depth varies by field**: Jp(3), F(4), C(3), S(3) — only F benefits from 4 layers
6. **Prior configs don't transfer**: S field prior optimal (1280×4, omega=50) failed on new dataset
...
| 4 | S | 100 | 37–48 | In progress | 0.574 |
